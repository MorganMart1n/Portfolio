import { Component, ElementRef, ViewChild, afterNextRender, Inject, PLATFORM_ID, HostListener } from '@angular/core';
...
export class App {
  ...
  // ========== NEW CONSTRUCTOR WITH FULL IMPORTS ==========
constructor(@Inject(PLATFORM_ID) private platformId: Object) {
  if (isPlatformBrowser(this.platformId)) {
    // Use setTimeout instead of afterNextRender
    setTimeout(() => {
      this.initAnimation();
...
scrollToElement(elementId: string): void {
  ...
  if (element) {
    ...
    // This sets just the hash, ignores the path
  ...
  updateActiveSection() {
    ...
    for (const section of sections) {
      ...
      if (element) {
        ...
        // Consider section active when it's within 100px of the top
